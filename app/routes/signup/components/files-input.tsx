...
export default function FilesInput({ filesMutation }: FilesInputProps) {
    return (
        <Root htmlFor="dropzone-file" className={`flex flex-col items-center justify-center h-64 border-2 ${filesMutation.isError ? 'border-red-500' : null} ${filesMutation.isSuccess ? 'border-green-600' : null} transition-colors border-gray-300 border-dashed rounded-xl cursor-pointer hover:text-white hover:bg-cerulean dark:hover:bg-prussian-blue`}>
